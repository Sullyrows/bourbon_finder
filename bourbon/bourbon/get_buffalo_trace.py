...
async def buffalo_trace_avail(my_page: Page) -> pd.DataFrame:
    await my_page.goto(
        "https://www.buffalotracedistillery.com/product-availability"
    )
    ...
    for ind, ele in enumerate(ele_frame):
        ...
        await my_page.wait_for_selector(
            selector="div.product-availability-icon", timeout=2000
        )
        ...
        product_avail_flag = (
            computed_value
            == 'url("https://www.buffalotracedistillery.com/content/dam/buffalotracedistillery/landing-pages/product-availability/images/product-availability-available.png")'
        )
